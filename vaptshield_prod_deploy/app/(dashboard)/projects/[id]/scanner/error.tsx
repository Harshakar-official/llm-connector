...
export default function ProjectScannerError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Project scanner error:", error)
  }, [error])
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 p-6">
      ...
      <h1 className="text-xl font-semibold text-fg">Scanner Failed to Load</h1>
      <p className="text-sm text-fg-muted text-center max-w-md">
        {error.message || "An unexpected error occurred while loading the scanner. Please try again."}
      </p>
      <button
        onClick={reset}
